...
def pick_random_question():
    ...
    # Console debug only
    print(f"[DEBUG] Paper: {paper}, Qnum: {qnum}, Path: {qpath}, Answer: {ans}")
...
async def ask_question(ctx, user_id):
    ...
    def check(reaction, user):
        nonlocal stopped
        if reaction.emoji == stop_emoji and (user.guild_permissions.manage_guild or user == ctx.guild.owner):
            stopped = True
            return True
...
async def competitive_mode(ctx):
    ...
    await ctx.send(f"🏁 Competitive round started! Race to be the first to answer; reach for the summit of Physics!  Paper: **{paper}**\nMods can react ❌ at any question to stop.")
    ...
    for qnum in qkeys:
        ...
        qpath = database[paper]["questions"][qnum]
        correct = database[paper]["answers"][qnum]["answer"]
...
async def quiz(ctx):
    try:
        ...
        print(f"[ERROR] ask_question failed: {e}")
